...
export function getCameraLocationDescription(cameraId, fallbackUrl = '') {
  const cleanId = String(cameraId).trim();
  if (CAMERA_LOCATION_MAP[cleanId]) {
    return CAMERA_LOCATION_MAP[cleanId];
  }

  // Backup Parsing Strategy if LTA adds a new camera not present in Annex G
  if (fallbackUrl) {
    const match = fallbackUrl.match(/_([A-Z0-9]+)_[0-9]+/);
    if (match && match[1]) {
      return `${match[1]} (Camera ${cleanId})`;
    }
  }

  return `Singapore Location (Camera ${cleanId})`;
}

export function enrichCameras(rawCameras = []) {
  return rawCameras.map(cam => ({
    ...cam,
    locationDescription: getCameraLocationDescription(cam.id, cam.imageUrl)
  }));
}